...
import WalletDepositAmountModal from './WalletDepositAmountModal'
import WalletDepositConfirmModal from './WalletDepositConfirmModal'
...
export default function TenantTable(props) {
    const [walletDepositAmountModalOpen, setWalletDepositAmountModalOpen] = useState(false);
    const [depositAmount, setDepositAmount] = useState('');
    const [walletDepositConfirmModalOpen, setWalletDepositConfirmModalOpen] = useState(false);
    let data = props.data
    ...
    const depositMutation = useMutation(() => {
        return axios.post(appConfig.SERVER_URL + `/v1/wallets/deposit`,
            {
                "amount": Number(depositAmount),
                "tenant_ids": (selectedFlatRows.map(d => d.original)).map(obj => obj.ID)
            },
            {
                headers: {
                    "Content-type": "application/json",
        ...
        {
            onError: (error, variables, context) => {
                alert.log("did not work")

            },
            onSuccess: (data, variables, context) => {
                setWalletDepositConfirmModalOpen(false);
                alert.log("worked")
            },
        }
    );
    ...
    // mutation.mutate({ "tenant_ids": (selectedFlatRows.map(d => d.original)).map(obj => obj.ID) })
    ...
    return (
        <>
            <WalletDepositAmountModal open={walletDepositAmountModalOpen} setOpen={setWalletDepositAmountModalOpen} depositAmount={depositAmount} setDepositAmount={setDepositAmount} setOpenConfirmModal={setWalletDepositConfirmModalOpen} />
            <WalletDepositConfirmModal open={walletDepositConfirmModalOpen} setOpen={setWalletDepositConfirmModalOpen} depositAmount={depositAmount} selectedRows={selectedFlatRows} mutate={depositMutation.mutate} />
                ...
                <div className="sm:flex sm:items-center">
                    ...
                    {selectedFlatRows.length > 0 ?
                        <div className="grid grid-cols-2 gap-8">
                            <button
                                onClick={() => setWalletDepositAmountModalOpen(true)}//mutation.mutate({ "tenant_ids" : selectedFlatRows.map((row) => row.ID) })}
                                type="button"
