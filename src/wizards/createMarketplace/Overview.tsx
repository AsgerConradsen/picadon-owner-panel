...
import { MarketplaceCreatedModal } from './components/MarketplaceCreatedModal'
import { FieldValues, UseFormHandleSubmit, UseFormWatch } from 'react-hook-form'
...
interface Props {
    errors: { [x: string]: any; },
    handleSubmit: UseFormHandleSubmit<FieldValues>, //UseFormHandleSubmit<FieldValues>,
...
export const Overview: React.FC<Props> = (props) => {
    const watchName = props.watch("name", "");
    const watchMarketplaceWebAddress = props.watch("marketplaceWebAddress");
    const watchEmail = props.watch("email", "");
    const watchPhoneNumber = props.watch("phoneNumber", "");
    return (
        <div>
            <MarketplaceCreatedModal open={props.modalOpen} />
            {<Sidebar steps={steps} />}
                    ...
                    <dl className="divide-y divide-gray-200">
                        <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4">
                            ...
                            <dd className="mt-1 flex text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                <span className="flex-grow">{watchName}</span>
                        ...
                        <div className="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4">
                            <dt className="text-sm font-medium text-gray-500">Web address</dt>
                            <dd className="mt-1 flex text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                <span className="flex-grow">{"app.picadon.io/" + watchMarketplaceWebAddress}</span>
                                <span className="ml-4 flex-shrink-0">
                                    <Link
                                        to="/register/account-info"
                                        className="bg-white rounded-md font-medium text-purple-600 hover:text-purple-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500"
